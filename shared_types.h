...
#define MESUREMENT_FREQUENCY 1920 // Full IMU performance = 1920Hz
...
#define MSG_ID_STATS 0x12         // Processing rate statistics
...
/**
 * @brief Processing rate statistics (SubCores -> MainCore)
 */
typedef struct
{
  uint8_t core_id;           // Which SubCore sent this
  float actual_rate_hz;      // Actual processing rate in Hz
  uint32_t loop_count;       // Total number of loops processed
  uint32_t dropped_messages; // Number of dropped messages (if any)
} ProcessingStats_t;
